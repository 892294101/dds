...
func (h helpCmd) Exec(ctx context.Context, args []string) (context.Context, error) {
	...
	if len(args) > 1 {
		cmdNameParam = strings.ToUpper(args[1])
	}

	// print help for a specified command
	if cmdNameParam != "" {
		cmd, found := commands[LibType][cmdNameParam]
		if !found {
			str := fmt.Sprintf("command %s not found\n", cmdNameParam)
...
func (e exitCmd) Exec(ctx context.Context, args []string) (context.Context, error) {
	out := api.GetStdout(ctx)
	fmt.Fprintf(out, "\n")
	os.Exit(0)
	return ctx, nil
...
func (p promptCmd) Exec(ctx context.Context, args []string) (context.Context, error) {
	if len(args) < 2 {
...
func (s sysinfoCmd) Exec(ctx context.Context, args []string) (context.Context, error) {
	out := api.GetStdout(ctx)
...
func (local localSys) Exec(ctx context.Context, args []string) (context.Context, error) {
